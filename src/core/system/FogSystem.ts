...
import type { Point, Segment, Polygon } from "../../utils/visibility_polygon.js";
...
export class FogSystem {
  ...
  setRenderMode(mode: 'grid' | 'polygon') {
    if (this.renderMode !== mode) {
      ...
      console.log(`[FogSystem] 渲染模式切换为: ${mode === 'grid' ? '网格采样' : '传统多边形'}`);
    }
  }
  ...
  private generatePlayerPositionKey(units: Unit[]): string {
    ...
    return playerUnits
      .map(u => `${u.id}:${Math.floor(u.x / (GRID_SIZE * RERENDER_DIFF_THRESHOLD))},${Math.floor(u.y / (GRID_SIZE * RERENDER_DIFF_THRESHOLD))}`)
      .sort()
      .join("|");
  }
  ...
  private calculateVisibleCells(
    ...
    for (let gx = 0; gx < gridWidth; gx++) {
      for (let gy = 0; gy < gridHeight; gy++) {
        const visibility = this.calculateCellVisibility(gx, gy, visibilityPolygons);
  ...
  makeFogOfWar = (
    ...
    const map = golbalSetting.map;
    if (map) {
      const currentPositionKey = this.generatePlayerPositionKey(map.sprites as Unit[]);
    ...
    if (visiblePointsArr.length > 0) {
      ...
      const shrunkPolygons = allVisibilityPolygons.map(poly => shrinkPolygon(poly, shrinkAmount));
      
      // 计算可见的方格（网格采样法）
      const visibleCells = this.calculateVisibleCells(shrunkPolygons, mapWidth, mapHeight);
      ...
      // 复用或创建 Canvas
      if (!this.fogCanvas || this.fogCanvas.width !== mapWidth || this.fogCanvas.height !== mapHeight) {
